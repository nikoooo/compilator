...
sym_index ast_integer::generate_quads(quad_list &q)
{
    ...
    // create temp var
    sym_index si = sym_tab->gen_temp_var(integer_type);

    quadruple *qq = new quadruple(q_iload, value, NULL_SYM, si);
...
sym_index ast_real::generate_quads(quad_list &q)
{
    ...
    // create temp var
    sym_index si = sym_tab->gen_temp_var(real_type);

    quadruple *qq = new quadruple(q_rload, sym_tab->ieee(value), NULL_SYM, si);
...
sym_index do_binaryoperation(quad_list &q, quad_op_type int_quad, quad_op_type real_quad, ast_binaryoperation *astbin){

    sym_index op1 = astbin->left->generate_quads(q);
    sym_index op2 = astbin->right->generate_quads(q);
    ...
    if (ltype != rtype && !(ltype == void_type || rtype == void_type) )
    {
        ...
        if (ltype == integer_type) {
            q += new quadruple(q_itor, op1, NULL_SYM, temp);
            op1 = temp;
        }
        else { // ==> (rtype == integer_type) 
            q += new quadruple(q_itor, op2, NULL_SYM, temp);
            op2 = temp;
        }
    }

    sym_index dest = sym_tab->gen_temp_var(astbin->type);
    if (astbin->type == integer_type) {
        q += new quadruple(int_quad, op1, op2, dest); 
    }
    else if (astbin->type == real_type)
    {
        q += new quadruple(real_quad, op1, op2, dest); 
    }

    return dest;
}
sym_index do_binaryrelation(quad_list &q, quad_op_type int_quad, quad_op_type real_quad, ast_binaryrelation *astbin){
    ...
    sym_index dest = sym_tab->gen_temp_var(integer_type);
    if (dest_type == integer_type) {
        q += new quadruple(int_quad, op1, op2, dest); 
    }
    else
    {
...
sym_index ast_not::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    sym_index p = expr->generate_quads(q);
    sym_index si = sym_tab->gen_temp_var(integer_type);
    quadruple *qq = new quadruple(q_inot, p, NULL_SYM, si);
...
sym_index ast_uminus::generate_quads(quad_list &q)
{
    ...
    sym_index si = sym_tab->gen_temp_var(expr->type);

    quadruple *qq;

    if (expr->type == integer_type) {
        qq = new quadruple(q_iuminus, p, NULL_SYM, si);
    } else if(expr->type == real_type) {
        qq = new quadruple(q_ruminus, p, NULL_SYM, si);
    }
...
sym_index ast_cast::generate_quads(quad_list &q)
{
    ...
    quadruple *qq = new quadruple(q_itor, p, NULL_SYM, si);
...
sym_index ast_divide::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binaryoperation(q, q_rdivide, q_rdivide, this); // TODO 
}
...
sym_index ast_idiv::generate_quads(quad_list &q)
{
    USE_Q;
    /* Your code here */
    return do_binaryoperation(q, q_idivide, q_idivide, this); // TODO
}
...
sym_index ast_if::generate_quads(quad_list &q)
{
    ...
    int endif   = sym_tab->get_next_label();
    
    sym_index con = condition->generate_quads(q);
    // if (...)

    if (elsif_list != NULL || else_body != NULL)
        q += new quadruple(q_jmpf, bottom, con, NULL_SYM);
    else
        q += new quadruple(q_jmpf, endif, con, NULL_SYM);

    // ...statements...
    body->generate_quads(q);
    // always go to end of all elsifs / elses when entering one
    // elsif
    if (elsif_list != NULL || else_body != NULL) {
        q += new quadruple(q_jmp, endif, NULL_SYM, NULL_SYM);
        q += new quadruple(q_labl, bottom, NULL_SYM, NULL_SYM);
    }
    
    if (elsif_list != NULL) {
        elsif_list->generate_quads_and_jump(q, endif);
    }
    ...
    // end of all ifs/elsifs/elses
    q += new quadruple(q_labl, endif, NULL_SYM, NULL_SYM);
...
sym_index ast_indexed::generate_quads(quad_list &q)
{
    ...
    sym_index j = sym_tab->gen_temp_var(id->type);
    if (id->type == integer_type) {
